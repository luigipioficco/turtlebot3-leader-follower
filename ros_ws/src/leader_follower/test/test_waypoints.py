...
# the margin on top is what keeps the target from grazing an obstacle while
# Nav2's own planner and controller settle onto the route.
ROBOT_RADIUS = 0.09
MARGIN = 0.15
MIN_CLEARANCE = ROBOT_RADIUS + MARGIN

# Nav2's planner tolerance on an intermediate waypoint is 0.15 m
# (planner_server.GridBased.tolerance in config/nav2_overrides.yaml): the
# planner is free to consider a waypoint satisfied from up to that far away.
# A leg much shorter than that leaves the planner free to cut the waypoint
# rather than actually visit it -- exactly the failure mode this project hit
# once, on a real waypoint placed close to a fixed marker. Twice the
# tolerance is a comfortable margin.
MIN_LEG = 2 * 0.18
...
def test_legs_are_long_enough(name):
    ...
    for i, (x0, y0) in enumerate(wps):
        ...
        assert length > MIN_LEG, \
            ...
            f"shorter than {MIN_LEG:.2f}: Nav2's planner tolerance could let it " \
            f"skip this waypoint instead of visiting it"
