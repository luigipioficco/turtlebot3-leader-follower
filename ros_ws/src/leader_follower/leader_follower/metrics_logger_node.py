# Records to CSV everything needed to compare the two strategies. Every run
# appends to the same two files (see __init__ below) rather than creating a
# new pair: run_id tells the rows of different runs apart.
import atexit
import csv
import math
import os
...
# install_shutdown_logging is adapted from evannsmc/ROS2Logger
# (github.com/evannsmc/ROS2Logger, ros2_logger/shutdown_helpers.py,
# MIT licence per the repository's README). Original idea: register the
# same "write the log and destroy the node" callback on SIGINT, SIGTERM and
# atexit, guarded so it runs exactly once no matter which of the three
# fires first.
#
# Why this matters here: rclpy.spin() only turns SIGINT into a
# KeyboardInterrupt (Python's own default handler); SIGTERM has no handler
# at all by default and kills the process immediately, skipping every
# try/finally. A run stopped that way would lose its summary row. The
# duration-based shutdown in check_duration() does not depend on this —
# raising SystemExit already returns from spin() normally — but a bare
# `kill` (SIGTERM, no -INT) on this node, or a shutdown that reaches it as
# SIGTERM, previously did not.
#
# Adapted from the original: no separate Logger class (this node already
# does its own CSV writing) and no also_shutdown hook (not needed here);
# kept: the lock-guarded run-once flag and the three trigger points. Returns
# run_once itself, so main()'s own explicit finally-block call goes through
# the same guard as the signal/atexit ones — calling finalize() directly
# there, instead, would bypass the guard entirely and run it twice on every
# ordinary (duration-reached) exit.
def install_shutdown_logging(finalize):
    lock = threading.Lock()
    done = False

    def run_once(_sig=None, _frame=None):
        nonlocal done
        with lock:
            if done:
                return
    ...
    signal.signal(signal.SIGINT, run_once)
    signal.signal(signal.SIGTERM, run_once)
...
class MetricsLogger(Node):
    ...
    def __init__(self):
        ...
        # Two FIXED files, shared across every run, appended rather than
        # created fresh: run_id (label + timestamp) is a column, not part of
        # the filename. A campaign of days of testing (Nav2 debugging
        # included) still produces two files, not one pair per launch.
        self.run_id = f"{self.label}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.samples_path = os.path.join(self.out_dir, 'samples.csv')
        self.summary_path = os.path.join(self.out_dir, 'summary.csv')

        new_file = not os.path.exists(self.samples_path)
        self.fh = open(self.samples_path, 'a', newline='')
        ...
        if new_file:
            self.csv.writerow(['run_id', 't', 'state', 'visible', 'meas_distance',
    ...
    def write_row(self, rel):
        ...
        self.csv.writerow([self.run_id, f"{rel:.3f}", self.state, int(vis),
                           f"{md:.4f}", f"{derr:.4f}",
    ...
    def compute_summary(self):
        ...
        return {
            ...
            'mean_recovery_time_s': self.mean(self.recoveries),
    ...
    def save_summary(self, summary):
        ...
        self.get_logger().info(f"summary appended to {self.summary_path}")
...
def main(args=None):
    ...
    finalize_once = install_shutdown_logging(finalize)
    try:
        rclpy.spin(node)
    except (KeyboardInterrupt, SystemExit):
        pass          # SystemExit: simulated duration reached, not an error
    finally:
        finalize_once()   # same guard as the signal/atexit paths: runs once
