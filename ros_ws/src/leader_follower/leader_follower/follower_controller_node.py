...
class FollowerController(Node):
    ...
    def arbitrate_reverse(self, v, w):
        # The same graded law applied to the rear sector. The Burger's LiDAR
        # spans a full 360 degrees, so the space behind is known as well as the
        # space ahead. Rotation is never limited, so the target stays framed.
        d_min = self.range_towards(math.pi, half_width=self.front_half)
    ...
    def freer_side(self, ang, r, ok):
        """+1 when there is more room to the left, -1 to the right."""
        left = ok & (ang > 0.0) & (ang < math.pi / 2)
        right = ok & (ang < 0.0) & (ang > -math.pi / 2)
    ...
    def front_obstacle(self):
        ...
        front = ok & (np.abs(ang) < self.front_half)
        if not np.any(front):
            return float('inf'), 1.0
        return float(np.min(r[front])), self.freer_side(ang, r, ok)
