...
#
# The node subscribes to the camera stream, looks for the marker carrying the
# expected identifier, estimates its pose relative to the camera and publishes
# it in the fixed odom frame.
import numpy as np
import cv2
import rclpy
...
class TargetDetector(Node):
    ...
    def on_image(self, msg):
        ...
        idx = self.index_of_target(ids)

        found = False
        if idx is not None:
            found = self.publish_target(corners[idx], msg)

        # The flag is published ALWAYS, even with no marker in sight: it is the
        # only topic that states non-visibility explicitly.
        self.pub_visible.publish(Bool(data=idx is not None))
    ...
    def index_of_target(self, ids):
        # Four markers of the same dictionary exist in the arena: the three
        # fixed panels and the target. More than one is often visible in a
        # single frame, and this filter is what makes the follower pursue its
        # own target rather than the nearest wall panel. The main trajectory
        # deliberately runs alongside all three so that it can be seen working.
        if ids is None:
            return None
        flat = ids.flatten().tolist()
        return flat.index(self.target_id) if self.target_id in flat else None
