...
Arguments:
    ...
    run_label:=<name>       prefix of the CSV files produced
...
def launch_setup(context, *args, **kwargs):
    ...
    # Target speed, overridable from the command line. At 0.21 m/s against a
    # follower limited to 0.22, the recovery margin is only 0.01 m/s, so a
    # strategy that stops never closes the gap again. This is the condition
    # Challenge 2 of the proposal describes.
    driver_params = {'use_sim_time': True, 'trajectory': trajectory}

    speed = LaunchConfiguration('target_speed').perform(context)
    if speed:
        driver_params['max_linear'] = float(speed)

    driver = Node(
        package='leader_follower', executable='target_driver_node',
        name='target_driver_node', output='screen',
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description='following distance in m; empty = value from the '
                        'configuration file'),
        DeclareLaunchArgument(
            'target_speed', default_value='',
            description='target speed in m/s; empty = value from the '
                        'configuration file'),
